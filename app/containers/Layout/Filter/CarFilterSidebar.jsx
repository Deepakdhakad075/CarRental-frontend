...
const CarFilterSidebar = ({ onApply }) => {
  ...
  return (
    <div className="bg-gray-100 p-4 w-full md:w-64 rounded-lg shadow-sm h-auto overflow-y-auto">
     ...
     {/* Brands */}
<div className="mb-4">
  <h4 className="text-sm font-semibold mb-2 text-gray-700">Brand</h4>
  {['Toyota', 'Honda', 'Kia', 'Mahindra', 'Maruti'].map((brand) => (
    <label key={brand} className="flex items-center space-x-2 mb-1">
      <input
        type="checkbox"
        checked={filters.models.includes(brand)}
        onChange={() => handleToggle('models', brand)} // Keep using 'models' key in Redux
        className="accent-indigo-600"
      />
